...
def upload_to_company_image(instance, filename):
    company_name = instance.company_ref.company_name
    return f'companies/{company_name}/{filename}'
...
class Product(models.Model):
    manufacturer = models.ForeignKey(Company, on_delete=models.CASCADE, db_index=True)
    part_number = models.CharField(max_length=20, blank=True, unique=True, db_index=True)
    fan_type = models.ForeignKey(Fantype, blank=True, on_delete=models.CASCADE, related_name="fantype", null=True)
    ac_dc = models.CharField(max_length=4, choices=[("AC","AC"), ("DC","DC")])
    length = models.IntegerField()
    width = models.IntegerField()
